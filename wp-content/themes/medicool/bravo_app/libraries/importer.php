        ...
        static function _do_import()
        {
            if(isset($_REQUEST['bravo_do_import']) and $_REQUEST['bravo_do_import'])
            {
                ...
                if(!current_user_can('manage_options')){
                    echo json_encode(array('status'=>0,'message'=>esc_html__('You do not have permission to do this','medicool')));die;
                }
                ...
                if ( !class_exists( 'WP_Importer' ) or ! class_exists( 'WP_Import' ) ){
                    echo json_encode(array('status'=>0,'message'=>esc_html__('Importer Class Was Not Installed','medicool')));die;
                }
                ...
                if($step==1) {
                    ...
                    if(!is_wp_error($data_res))
                    {
                        ...
                        if(!empty($options)){
                            ...
                            if(!function_exists('ot_options_id'))
                            {
                                echo json_encode( array(
                                        'status'   =>0,
                                        'messenger'=>esc_html__("Plugin: Option Tree must be installed first. Stop working!",'medicool'),
                                        'next_url' => ''
                                    )
                                );
                                die;
                            ...
                            echo json_encode( array(
                                    'status'   =>"ok",
                                    'messenger'=>esc_html__("Importing the demo theme options... ",'medicool')."<span>DONE!</span><br>",
                            ...
                            echo json_encode( array(
                                    'status'   =>0,
                                    'messenger'=>esc_html__("File: theme_options.json contain NULL content. Stop working!",'medicool'),
                                    'next_url' => ''
                                )
                            );
                        }
                        ...
                        echo json_encode( array(
                                'status'   =>0,
                                'messenger'=>sprintf(esc_html__("<span class='red'>Can not read theme_options.json<br>File:%s<br>. Stop working!</span>",'medicool'),$data_json),
                                'next_url' => ''
                            )
                        );
                        die;
                ...
                if($step==2){
                    ...
                    echo json_encode( array(
                            'status'   =>1,
                            'messenger'=>esc_html__("Importing the demo widgets... ",'medicool').'<span>DONE!</span>.<br>',
                ...
                if($step==3){
                    ...
                    if(!$file_name){
                        echo json_encode( array(
                                'status'   =>0,
                                'messenger'=>esc_html__("File Not Found. Stop working!",'medicool'),
                                'next_url' => ''
                            )
                        );
                        die;
                    ...
                    echo json_encode( array(
                            'status'   =>1,
                            'messenger'=>sprintf(esc_html__("Importing data: %s  of %d ... ",'medicool'),basename($file_name)." ".($stt_file+1),(count($ds_file)))."<span>DONE!</span><br>",
                            'next_url' => $nexturl,
                            'file'     => $ds_file,
                        )
                    );
                ...
                if($step==4) {
                    ...
                    echo json_encode(array(
                            'status' => 1,
                            'messenger' => esc_html__("Importing menu settings ... ", 'medicool')."<span>DONE!</span><br>",
                            'next_url' => $nexturl,
                        )
                    );
                }
                ...
                if($step == '5'){
                    ...
                    echo json_encode( array(
                            'status'   =>"ok",
                            'messenger'=>esc_html__("Setting reading options... ",'medicool')."<span>DONE!</span><br/><span>All Done! Have Fun</span>",
                            'next_url' => '',
                        )
                    );
